...
if [ -z "$API_APP_ID" ]; then
  API_APP_ID=$(az ad app create --display-name "Stability Capture API ($ENV)" --sign-in-audience AzureADMyOrg --app-roles @"$APP_ROOT/backend/infra/entra-app-roles.json" --query appId -o tsv)
...
az deployment group create -g "$RG" -f "$APP_ROOT/backend/infra/main.bicep" \
...
echo "== 4/6 Package and deploy (dist + sdct/backend/api)"
