...
The mockup is the production bundle with `VITE_API_BASE` empty. The same screens, validation, branching and naming run against an in-memory API with 120 seeded observations, and the Express API has a matching LOCAL_MODE. Reviewers can exercise the whole flow with no tenant and no storage account.
