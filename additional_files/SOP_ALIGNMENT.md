...
The rule lives once, in `src/lib/ratings.js`, and the demo data is seeded through the same function. Ratings are stored as text codes `0` to `5` (vocabularies `RATING_CREAMING`, `RATING_SERUM`, `RATING_SEDIMENT`) so labels can carry the SOP wording.
